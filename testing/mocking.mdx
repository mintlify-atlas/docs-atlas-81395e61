...
description: Learn how to mock modules, APIs, and timers in Node.js tests using the built-in test runner's mock utilities.
---
...
<Note>
  "Mocks" and "stubs" are technically different kinds of "test doubles". A stub is a replacement that does nothing (a no-op) but tracks its invocation. A mock is a stub that also has a fake implementation (the "when 'a', do 'b'"). Within this doc, the difference is unimportant, and stubs are referred to as mocks.
</Note>
...
| end-to-end (e2e) | app + external data stores, delivery, etc | A fake user (e.g., a Playwright agent) literally using an app connected to real external systems. | none (do not mock) |
...
**Why mock:** For a true unit test of `main`, `foo` should be mocked — you're testing that `main` works, not that `main` + `foo` work (that's a different test).

**Why not mock:** Mocking `foo` can be more trouble than worth, especially when `foo` is simple, well-tested, and rarely updated. Not mocking `foo` can be better because it's more authentic and increases coverage of `foo` (because `main`'s tests will also verify `foo`). This can, however, create noise: when `foo` breaks, a bunch of other tests will also break, so tracking down the problem is more tedious.
...
**Why mock:** Verifying that code that your project does not maintain works is not the goal of a unit test (and that code should have its own tests).

**Why not mock:** Sometimes, it's just not realistic to mock. For example, you would almost never mock a large framework such as React or Angular (the medicine would be worse than the ailment).
...
A little-known fact is that there is a built-in way to mock `fetch`. [`undici`](https://github.com/nodejs/undici) is the Node.js implementation of `fetch`. It's shipped with Node.js, but not currently exposed by Node.js itself, so it must be installed (e.g., `npm install undici`).
...
<Note>
  Note the use of time-zone here (`Z` in the timestamps). Neglecting to include a consistent time-zone will likely lead to unexpected results.
</Note>
