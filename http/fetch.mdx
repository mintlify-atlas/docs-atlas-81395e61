---
title: 'Fetch API with Undici'
description: 'Learn how to use the Fetch API in Node.js, powered by Undici — including GET and POST requests, connection pooling, and response streaming.'
---
...
Undici allows you to customize the Fetch API by providing options to the `fetch` function. For example, you can set custom headers, set the request method, and set the request body.
...
In the following example, we are sending a POST request to the Ollama API with a JSON payload. Ollama is a CLI tool that allows you to run LLMs (Large Language Models) on your local machine. You can download it [here](https://ollama.com/download).
...
<Note>
  Always call `body.dump()` when you receive a non-success response and don't intend to read the body. Undici requires the response body to be consumed for proper garbage collection.
</Note>
